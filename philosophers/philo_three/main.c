...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2020/01/28 19:21:34 by nklarsfe          #+#    #+#             */
/*   Updated: 2020/02/06 18:30:25 by nicolas          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void init(int argc, char **argv){
	...
	for(i = 0; i < params->n; i++){
		...
		params->philosophers[i].is_eating = 
			ft_sem_open(getName(buf, SEM_NAME_IS_EATING, i), 1);
	}
	tmp = get_time_abs_us();
	for(i = 0; i < params->n; i++){
...
void clean(){
	...
	for(i = 0; i < params->n; i++){ 
		...
		sem_unlink(getName(buf, SEM_NAME_IS_EATING, i));
	...
	for(i = 0; i < params->n; i++){
		if(kill(params->philosophers[i].pid_my_life, 0) == 0){
			kill(params->philosophers[i].pid_my_life, SIGKILL);
		}
	}
	for(i = 0; i < params->n; i++){
		waitpid(params->philosophers[i].pid_my_life, NULL, 0);
	}

	if(kill(params->pid_waitForEverybody, 0) == 0){
		kill(params->pid_waitForEverybody, SIGKILL);
	}
	waitpid(params->pid_waitForEverybody, NULL, 0); // WNOHANG
}
...
void philo_eat(t_philosopher *philo){
	...
	wait_us(philo->last_time_started_to_eat, params->time_to_eat_us);	
	sem_post(philo->is_eating);
	// finish eating
...
// ./philosophers 7 20000 1000 1000 1
// ./philosophers 7 20000 10000 8000
int main(int argc, char **argv){
	t_params params_;
	int i;
	params = &params_;
	init(argc, argv);
	params->pid_waitForEverybody = fork();
	if (params->pid_waitForEverybody < 0)
		return (1);
	else if (params->pid_waitForEverybody == 0) {
		waitForEverybodyToHaveEatenEnoughTimes(NULL);
		exit(0);
	}
	for(i = 0; i < params->n; i++){ 
		params->philosophers[i].pid_my_life = fork();
		if (params->philosophers[i].pid_my_life < 0)
			return (1);
		else if (params->philosophers[i].pid_my_life == 0) {
			philosopherLife(&params->philosophers[i]);
			exit(0);
		}
		usleep(100);
	}
