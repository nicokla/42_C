...
int				check_test_errors(char **argv, int ac)
{
	...
	if (ac > 4)
	{
		ft_dprintf(2, "minishell: test: too many arguments\n");
		ret = 2;
	}
	else if (ac == 3 && get_unary_option(argv[1]) == -1)
	{
		ft_dprintf(2, "minishell: test: %s: unary operator expected\n", argv[1]);
		ret = 2;
	}
	else if (ac == 4 && get_binary_option(argv[2]) == -1)
	{
		ft_dprintf(2, "minishell: test: %s: binary operator expected\n", argv[2]);
		ret = 2;
	}
	return (ret);
}
