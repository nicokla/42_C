...
void *mlx_new_window(mlx_ptr_t *mlx_ptr, int *psize_x, int *psize_y, char *title)
{
  ...
  if (frame.size.width < *psize_x)
  {
	xx = frame.size.width;
	*psize_x = (int)frame.size.width;
  } 
  else
    xx = *psize_x;
  if (frame.size.height < *psize_y)
  {
	yy = frame.size.height;
	*psize_y = (int)frame.size.height;
  }
  else
    yy = *psize_y;
  NSRect windowRect = NSMakeRect(100, 100, xx, yy);
